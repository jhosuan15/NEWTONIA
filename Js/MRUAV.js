...
iniciarSimulacionBtn.addEventListener('click', () => {
    ...
    if (!isNaN(velocidadInicial) && !isNaN(aceleracion) && !isNaN(tiempo)) {
        ...
        // Establecer rotación y desplazamiento antes del movimiento
        if (totalDistancia >= 0) {
            objetoMRUA.style.transform = `translateY(-${totalDistancia}px) `; // Movimiento hacia arriba
        } else {
            objetoMRUA.style.transform = `translateY(${Math.abs(totalDistancia)}px)`; // Movimiento hacia abajo
        }
...
reiniciarSimulacionBtn.addEventListener('click', () => {
    objetoMRUA.style.transform = `translateY(0) `; // Reiniciar al centro con posición inicial
